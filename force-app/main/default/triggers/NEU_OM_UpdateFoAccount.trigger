trigger NEU_OM_UpdateFoAccount on Account (after insert) {
    ...
    for(Account obj : cuentas)
    {
        if(obj.Account_Executive_User__c!=null)
            users.add(obj.Account_Executive_User__c);
        if(obj.OwnerId!=null)
            users.add(obj.OwnerId);
        if(obj.Account_External_Follower_User__c!=null)
            users.add(obj.Account_External_Follower_User__c);
