trigger NEU_IE_Service_Update_Conversions on Import_Export_Fee_Line__c (before insert, before update)
{
    ...
    if(trigger.isUpdate)
    {
        ...
        for(Import_Export_Fee_Line__c fee_line:trigger.new)
        {
            Import_Export_Fee_Line__c old_fee_line = Trigger.oldMap.get(fee_line.Id);
            if((old_fee_line.Conversion_Rate_to_Currency_Header__c != fee_line.Conversion_Rate_to_Currency_Header__c) || Test.isRunningTest())
            {
                list_fee_lines.add(fee_line.Id);
            }
        }
        ...
        { 
            ...
            consulta_disbursement_lines += 'SELECT Id, Name, Conversion_Rate_to_Service_Line_Currency__c, Disbursement__r.Conversion_Rate_to_Imp_Exp_Currency__c, Import_Export_Service_Line__r.Conversion_Rate_to_Currency_Header__c ';
        ...
        for(Import_Export_Service_Line_Disbursement__c disbursement_line:query_disbursement_lines)
        {
            disbursement_line.Conversion_Rate_to_Service_Line_Currency__c = disbursement_line.Disbursement__r.Conversion_Rate_to_Imp_Exp_Currency__c / disbursement_line.Import_Export_Service_Line__r.Conversion_Rate_to_Currency_Header__c;
        }
