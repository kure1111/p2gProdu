trigger Pak_PAK50 on Customer_Quote__c (After insert) {
    ...
    if(!test.isRunningTest())
      {
            servicesRate =  [select id,Name,Fee_Rate__c,currencyIsoCode from Fee__c where Name =:name and Active__c = true ];
            system.debug('servicesRate '+ servicesRate);
        }
        else{ servicesRate =  [select id,Name,Fee_Rate__c,currencyIsoCode from Fee__c limit 10];} 
    
        if(servicesRate.size() > 0)
        {
            for(Customer_Quote__c cqc : trigger.new)
            {
                if(cqc.Account_for__c != null && cqc.Freight_Mode__c == 'Sea' && cqc.Service_Mode__c == 'IMPORT' || 
                   cqc.Freight_Mode__c == 'Sea' && cqc.Service_Mode__c == 'EXPORT'||
                   cqc.Freight_Mode__c == 'Air' && cqc.Service_Mode__c == 'IMPORT' && cqc.Service_Type__c == 'CARGA' ||
                   cqc.Freight_Mode__c == 'Air' && cqc.Service_Mode__c == 'EXPORT' && cqc.Service_Type__c == 'CARGA' ||
                   cqc.Freight_Mode__c == 'Road' && cqc.Service_Mode__c == 'PORT' && cqc.Service_Type__c == 'FTL' ||
                  cqc.Freight_Mode__c == 'Road' && cqc.Service_Mode__c == 'IMPORT' && cqc.Service_Type__c == 'FTL' ||
                  cqc.Freight_Mode__c == 'Road' && cqc.Service_Mode__c == 'EXPORT' && cqc.Service_Type__c == 'FTL' )
                {
            ...
            for(Customer_Quote__c cqc : trigger.new)
            {
                if(cqc.Freight_Mode__c == 'Sea' && cqc.Service_Mode__c == 'IMPORT' || 
                   cqc.Freight_Mode__c == 'Sea' && cqc.Service_Mode__c == 'EXPORT'||
                   cqc.Freight_Mode__c == 'Air' && cqc.Service_Mode__c == 'IMPORT' && cqc.Service_Type__c == 'CARGA' ||
                   cqc.Freight_Mode__c == 'Air' && cqc.Service_Mode__c == 'EXPORT' && cqc.Service_Type__c == 'CARGA' ||
                   cqc.Freight_Mode__c == 'Road' && cqc.Service_Mode__c == 'PORT' && cqc.Service_Type__c == 'FTL' ||
            ...
            for(Customer_Quote__c cqc : LstCustomerQuote)
            {
                ...
                servicesLine.currencyIsoCode  = servicesRate[0].currencyIsoCode;
