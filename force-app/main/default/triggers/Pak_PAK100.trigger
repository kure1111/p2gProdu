trigger Pak_PAK100 on Customer_Quote__c (after insert) {
    ...
    if(!test.isRunningTest())
      {
        ...
        if(servicesRate.size() > 0)
        {
            ...
            {
                if(cqc.Account_for__c != null && cqc.Freight_Mode__c == 'Road' && cqc.Service_Mode__c == 'NATIONAL' && cqc.Service_Type__c == 'TARIMAS' )
                {
            ...
            for(Customer_Quote__c cqc : trigger.new)
            {
                if(cqc.Freight_Mode__c == 'Road' && cqc.Service_Mode__c == 'NATIONAL' && cqc.Service_Type__c == 'TARIMAS')
                {
