trigger NEU_OM_UpdateFoServiceRate on Fee__c (after insert) {

    if(NEU_StaticVariableHelper.getBoolean1()){return;}		
    
    if(!RecursiveCheck.triggerMonitor.contains('NEU_OM_UpdateFoServiceRate')){
        RecursiveCheck.triggerMonitor.add('NEU_OM_UpdateFoServiceRate');
        Set<Id>users=new Set<Id>();
        Set<Id>accounts=new Set<Id>();
        
        List<Fee__c> service_rates= [select Id, Name,Account_for__c, Account_for__r.OwnerId, Account_for__r.Account_Executive_User__c, Account_for__r.Account_External_Follower_User__c, CreatedById, LastModifiedById from Fee__c  where Id IN:trigger.new];
            
        for(Fee__c obj : service_rates)
        {
        ...
        for(User qi: [select Id,AccountId from User where Id IN :users OR AccountId IN :accounts])
        {
            for(Fee__c obj:service_rates)
            {
                 if((obj.Account_for__r.OwnerId==qi.Id)||(obj.CreatedById==qi.Id) ||(obj.Account_for__r.Account_Executive_User__c==qi.Id)||(obj.Account_for__r.Account_External_Follower_User__c==qi.Id)
