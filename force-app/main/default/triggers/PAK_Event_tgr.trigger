trigger PAK_Event_tgr on Event (before insert, After insert, before update, After update) {
    ...
    if(trigger.isBefore){
        ...
        if(!contacto.isEmpty()){String Address = '';if(contacto[0].MailingAddress != null){Address = contacto[0].MailingStreet+' '+contacto[0].MailingCity+' '+contacto[0].MailingPostalCode+' '+contacto[0].MailingState+' '+contacto[0].MailingCountry;System.debug('--'+Address);}for(Event A: trigger.new){A.Contact_Address__c=Address;}}  
    }*/
}