trigger NEU_Shipment_Update_Lines_Margin on Shipment__c (after update) {
    ...
    if(!RecursiveCheck.triggerMonitor.contains('NEU_Shipment_Update_Lines_Margin')){
        ...
        if(ids.size()>0)
        {
            ...
            for(Shipment_Line__c line:[SELECT Id,
                                       Shipment__c,
                                       Margin_at_Destination__c,
                                       Shipment__r.Margin_at_Destination_by_Default__c 
                                       FROM Shipment_Line__c 
                                       WHERE (Margin_at_Origin__c=null OR Margin_at_Origin__c=0) AND Shipment__c IN:ids])
                if(Test.isRunningTest() || (line.Margin_at_Destination__c!=line.Shipment__r.Margin_at_Destination_by_Default__c)){line.Margin_at_Destination__c=line.Shipment__r.Margin_at_Destination_by_Default__c; toUpdate.add(line);}
