trigger NEU_OM_UpdateFoSupplyProject on Supply_Project__c (after insert) {
    ...
    List<Supply_Project__c> Supply_Project= [select Id, Name,Customer__c, Customer__r.Account_Executive_User__c, Customer__r.OwnerId, Customer__r.Account_External_Follower_User__c, CreatedById, LastModifiedById from Supply_Project__c where Id IN:trigger.new];
