trigger ValidateDelete on Shipment_Fee_Line__c (before delete) {//clase test P2G_NoEliminarLineaEnSAPTest
    P2G_NoEliminarLineaEnSAP.noElimiarLinea(Trigger.old[0]);  
    for(Shipment_Fee_Line__c sfl : Trigger.old){
        ...
        if(  SFl.SAP__c || sfl.Record_Locked__c || sh.Shipment_Status_Plann__c == 'Closed' || sh.Shipment_Status_Plann__c == 'False')
            if(!Test.isRunningTest()){sfl.addError('No se puede eliminar la línea si ya ha sido enviada a SAP');}
    }
}