...
export default class P2G_creacionFolioFleteInter extends LightningModal {
    isProduccion = true;
...
//Variables para busqueda
    ...
    //para Fraccion Arancelaria
    @track sideRecordsFraccionArancelaria;
    searchValueFraccionArancelaria ='';
    searchValueIdFraccionArancelaria ='';
    showSideFraccionArancelaria = false;
...
//Acciones de busqueda
    ...
    SideSelectContainerType(event){
        this.searchValueContainerType = event.currentTarget.dataset.name;
    ...
    SideSelectClaveUnidadPeso(event){
        this.searchValueClaveUnidadPeso = event.currentTarget.dataset.name;
    ...
    // buscador Fraccion Arancelaria
    SideSelectFraccionArancelaria(event){
        this.searchValueIdFraccionArancelaria = event.currentTarget.dataset.id;
        this.wrapperCargoLine.idFraccionArancelaria = event.currentTarget.dataset.id;
        this.searchValueFraccionArancelaria = event.currentTarget.dataset.name;
        this.showSideFraccionArancelaria = false;
    }
    searchKeyFraccionArancelaria(event){
        this.searchValueFraccionArancelaria = event.target.value;
        this.searchValueIdFraccionArancelaria='';
        if (this.searchValueFraccionArancelaria.length >= 3) {
            this.showSideFraccionArancelaria = true;
            getClaveSAT({sat: this.searchValueFraccionArancelaria,record: '5'})
                .then(result => {
                    this.sideRecordsFraccionArancelaria = result;
                })
                .catch(error => {
                    this.showToast('Error','error', error.body.message);
                    this.sideRecordsFraccionArancelaria = null;
                });
        }
        else{
            this.showSideFraccionArancelaria = false;
...
// guardar Registro
    ...
    registroServiceMode(event){
        this.wrapperFolio.serviceMode = event.target.value;
        this.valueServiceMode = event.target.value;
    }
    registroServiceType(event){
        this.wrapperFolio.servicetype = event.target.value;
        this.valueServiceType = event.target.value;
        if(this.valueServiceType === 'LTL'){
            this.siLTL = true;
        }else{
            this.siLTL = false;
        }
    }
    registroBorder(event){
        this.wrapperFolio.border = event.target.value;
    }
    registroCustoms(event){
        this.wrapperFolio.customs = event.target.value;
    }
    ...
    registroPedimento(event){
        this.wrapperCargoLine.pedimento = event.target.value;
    }
    registroUnits(event){
    ...
    camposRequeridos(){
        if(this.siLTL === true){
            const requiredFields = [
                this.wrapperFolio.serviceMode,
                this.wrapperFolio.servicetype,
                this.wrapperFolio.border,
                this.wrapperFolio.customs,
                this.searchValueIdAccount,
                this.searchValueIdSst,
                this.searchValueIdLoad,
                this.searchValueIdDischarge,
...
//clic de botones
    ...
    abrirCargoLine(){
        ...
        this.searchValueFraccionArancelaria = null;
        this.searchValueIdFraccionArancelaria = null;
        this.wrapperCargoLine.pedimento = null;
