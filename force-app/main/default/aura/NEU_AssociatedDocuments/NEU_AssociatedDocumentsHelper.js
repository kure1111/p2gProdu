({
    ...
    createAssociatedDocuments : function(cmp, ids_files) {
        var action = cmp.get('c.createAssociatedDocuments');
        action.setParams({recordId : cmp.get("v.recordId"), ids_ficheros : ids_files});
        action.setCallback(this, $A.getCallback(function (response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                cmp.set('v.documents', response.getReturnValue());
