...
(function($) {
    $.fn.NEUselect = (function(options) {
    	...
    	this.each(function(){
			...
			{
				if(select.attr('relatedTo'))
				{
					if(select.attr('relatedToMaster'))
						newvalue=$('option[value=\''+select.val()+'\']',$(select.attr('relatedTo')+' [master=\''+select.attr('relatedToMaster')+'\']')).text();
					else
						newvalue=$('option[value=\''+select.val()+'\']',$(select.attr('relatedTo'))).text();
				}
				if(!newvalue)
					input.val(select.val()).addClass('NEUerror');
				else
					input.val(newvalue).removeClass('NEUerror');
			}
    		select.hide();
    		select.after(input);
    	});
